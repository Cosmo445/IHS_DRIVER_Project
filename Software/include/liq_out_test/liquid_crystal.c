...
void lcd_drive_write(int fd, uint32_t data) {
   ...
   // int rs = ((data & 0b00000000000000000010000000000000) == 0) ? 0 : 1;
    ...
    //char caracter = ch;
   // printf("Descrição: en = '%d', rw = '%d', rs = '%d', on = '%d', symbol = '%c'", en, rw, rs, on, caracter);
...
// Adicione isso no início ou final do arquivo (fora de funções)
//__attribute__((used, section(".note.GNU-stack"))) static const char dummy_stack_note;
